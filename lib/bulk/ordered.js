...
OrderedBulkOperation.prototype.execute = function(_writeConcern, callback) {
  var self = this;
  if (this.s.executed) {
    var error = toError('batch cannot be re-executed');
    return (typeof callback === 'function') ? callback(error, null) : this.s.promiseLibrary.reject(error);
  }
  ...
  if (this.s.batches.length === 0) {
    var error = toError('Invalid Operation, no operations specified');
    return (typeof callback === 'function') ? callback(error, null) : this.s.promiseLibrary.reject(error);
  }
